...
$selected_keys = array_rand($all_katakana, 30);
...
foreach ($selected_keys as $roman) {
    $correct = $all_katakana[$roman];
    $options = [$roman];
    $wrong_options = array_diff(array_keys($all_katakana), [$roman]);
...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    foreach ($quiz_questions as $q) {
        if (isset($_POST[$q['katakana']]) && $_POST[$q['katakana']] == $q['correct']) {
            $score++;
        }
    }
...
<body>
    <div class="quiz-container">
        <h1>Gamification Quiz: Katakana Basics</h1>
        <p class="text-center">Test your knowledge of basic Katakana characters. Select the correct romanization for each character.</p>
        ...
        <form method="post">
            <?php foreach ($quiz_questions as $index => $q): ?>
                <div class="question">
                    <h3><?php echo ($index + 1) . ". What is the romanization for '" . $q['katakana'] . "'?"; ?></h3>
                    <div class="options">
                        <?php foreach ($q['options'] as $option): ?>
                            <label class="option"><input type="radio" name="<?php echo $q['katakana']; ?>" value="<?php echo $option; ?>" required> <?php echo $option; ?></label>
                        <?php endforeach; ?>
                    </div>
                </div>
            <?php endforeach; ?>
