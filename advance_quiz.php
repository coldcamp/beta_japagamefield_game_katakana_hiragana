...
// Advanced Hiragana and Katakana characters (voiced, combined, etc.)
$advanced_characters = [
...
// Remove duplicates (since some roman like 'ga' appear in both)
$advanced_characters = array_unique($advanced_characters);

// Select 30 random keys
$selected_keys = array_rand($advanced_characters, min(30, count($advanced_characters)));
...
foreach ($selected_keys as $roman) {
    $correct = $advanced_characters[$roman];
    $options = [$roman];
    $wrong_options = array_diff(array_keys($advanced_characters), [$roman]);
...
$_SESSION['advance_quiz_questions'] = $quiz_questions;
...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    $quiz_questions = $_SESSION['advance_quiz_questions'];
    foreach ($quiz_questions as $q) {
        if (isset($_POST[$q['character']]) && $_POST[$q['character']] == $q['correct']) {
            $score++;
        }
    }
...
<head>
    ...
    <title>Advanced Quiz - Hiragana and Katakana Learning</title>
...
<body>
    <div class="quiz-container">
        <h1>Advanced Quiz: Combined Sounds</h1>
        <p class="text-center">Test your knowledge of advanced Hiragana and Katakana characters, including voiced and combined sounds.</p>
        ...
        <form method="post">
            <?php foreach ($quiz_questions as $index => $q): ?>
                <div class="question">
                    <h3><?php echo ($index + 1) . ". What is the romanization for '" . $q['character'] . "'?"; ?></h3>
                    <div class="options">
                        <?php foreach ($q['options'] as $option): ?>
                            <label class="option"><input type="radio" name="<?php echo $q['character']; ?>" value="<?php echo $option; ?>" required> <?php echo $option; ?></label>
                        <?php endforeach; ?>
                    </div>
                </div>
            <?php endforeach; ?>
